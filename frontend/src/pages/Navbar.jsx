    ...
    <nav style={{ backgroundColor: '#171717', color: 'white', width: '100%', position: 'fixed', top: 0, left: 0, zIndex: 9999 }}>
      <ul style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 20px', margin: 0, fontSize: '16px' }}>
        <li style={{ listStyleType: 'none' }}>
          ...
          <Link to="/products" style={{ color: 'white', textDecoration: 'none', marginRight: '18px' }}>Courses</Link>
        ...
        <li className="user-info" style={{ listStyleType: 'none', fontSize: '16px' }}>
          {user ? (
            <>
              <span style={{ marginRight: '10px' }}>Welcome, {user.full_name}</span> {/* Assuming username is in user object */}
              <button onClick={() => setShowMenu(!showMenu)}> ☰ </button>
            </>
          ) : (
            <Link to="/signup" style={{ color: 'white', textDecoration: 'none', marginRight: '10px' }}>Sign Up</Link>
          )}
        </li>
      </ul>
      {showMenu && user && (
        <div style={{ backgroundColor: '#171717', color: 'white', padding: '10px', position: 'absolute', top: '50px', right: '20px' }}>
          <ul style={{ listStyleType: 'none', padding: 0 }}>
            <li style={{ marginBottom: '10px', fontSize: '16px' }}><Link to="/dashboard" style={{ color: 'white', textDecoration: 'none' }}>Profile</Link></li>
            <li><button style={{ backgroundColor: 'transparent', color: 'white', border: 'none', cursor: 'pointer', fontSize: '14px' }} onClick={handleLogout}>Logout</button></li>
          </ul>
        </div>
      )}
    </nav>
